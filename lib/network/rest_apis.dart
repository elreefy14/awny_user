...
//https://awnyapp.com/api/availability
// [8:20 ص، 2025/2/10] Mahmoud Elnagar: {
//     "longitude" : 4,
//     "latitude" : 1
// }
// [8:20 ص، 2025/2/10] Mahmoud Elnagar: {
...
Future<DashboardResponse> userDashboard({
  ...
  log('\n================================================');
  log('COUNTRY SELECTION DEBUG');
  log('================================================');
  log('User Country Code: $countryCode');
  log('Country Parameter: $country');
  log('================================================\n');
  ...
  try {
    ...
    log('================================================');
    log('URL: ${buildBaseUrl(endPoint)}');
    log('Method: GET');
    log('Parameters:');
    log('- isCurrentLocation: $isCurrentLocation');
    log('- latitude: $lat');
    log('- longitude: $long');
    log('- country: $country');
    ...
    // Print each main section with clear separation
    if (responseData['slider'] != null) {
      log('\n=== SLIDERS ===');
      log(JsonEncoder.withIndent('  ').convert(responseData['slider']));
    }

    if (responseData['category'] != null) {
      log('\n=== CATEGORIES ===');
      log(JsonEncoder.withIndent('  ').convert(responseData['category']));
    }

    if (responseData['service'] != null) {
      log('\n=== SERVICES ===');
      log(JsonEncoder.withIndent('  ').convert(responseData['service']));
    }
    ...
    if (responseData['upcoming_data'] != null) {
      log('\n=== UPCOMING BOOKINGS ===');
      log(JsonEncoder.withIndent('  ').convert(responseData['upcoming_data']));
    }

    if (responseData['notification_unread_count'] != null) {
      log('\n=== NOTIFICATION COUNT ===');
      log('Unread notifications: ${responseData['notification_unread_count']}');
    }
...
Future<List<PaymentSetting>> getPaymentGateways(
    {bool requireCOD = true, bool requireWallet = true}) async {
  try {
